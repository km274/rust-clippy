...
use clippy_utils::{SpanlessEq, sym as clippy_sym};
...
impl<'tcx> LateLintPass<'tcx> for SameLengthAndCapacity {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'_>) {
        if let ExprKind::Call(path_expr, args) = expr.kind
            ...
            && is_type_diagnostic_item(cx, cx.typeck_results().node_type(ty.hir_id), sym::Vec)
            && fn_path.ident.name == clippy_sym::from_raw_parts
